  ...
  return (
    <nav className='pagination is-small' role='navigation' aria-label='pagination'>
      {(() => {
        ...
        if (disabled === true) {
          return (
            <a className='pagination-previous' >Vorherige Frage</a>);
        } else {
          return (
            <a className='pagination-previous'
              href={previous !== undefined ? `/${RouteNames.questionnaire}/${previous}` : ''}
            >Vorherige Frage</a>);
        }
      })()}
      {(() => {
        ...
        if (disabled === true) {
          ...
          return (
            <a className='pagination-next'
              href={next !== undefined ? `/${RouteNames.questionnaire}/${next}` : ''}
            >Nächste Frage</a>);
        }
      })()}
      <ul className='pagination-list'>
        <li>
          <a className={(() => {
            ...
            href={`/${RouteNames.questionnaire}/${1}`} aria-label='Gehe zu Frage 1'
          >1</a>
        </li>
        {(() => {
          if (props.qId < 4) {
          ...
          if (props.qId < 2 || props.qId > props.lastId - 2) {
            ...
            return (<li>
              <a className='pagination-link is-current'
                aria-label={`Seite ${props.qId}`} aria-current='page'
              >{props.qId}</a>
            </li>);
          }
        })()}
        ...
        {(() => {
          const isCurrentClass = props.qId === props.lastId + 1 ? 'is-current' : '';
          return (
            <li>
              <a
